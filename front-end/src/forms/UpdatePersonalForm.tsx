import { type UpdateMemberToBusiness } from "@/types/Member.types";
...
export default function AddPersonnelForm() {
  const {
    ...
    formState: { errors },
    getValues,
  } = useFormContext<UpdateMemberToBusiness>();
  const {currentBusinessId} = useBusinessContext();
  const { data: branches, isLoading } = useBranches(currentBusinessId);

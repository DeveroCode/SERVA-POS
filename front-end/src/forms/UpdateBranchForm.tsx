...
import type { Business, updateBrach } from "@/types/Index.types";
import { Building2, MapPin, Phone, Mail, Link } from "lucide-react";
import { useFormContext } from "react-hook-form";

type UpdateBranchViewProps = {
  business: Business;
};

export default function UpdateBranchForm({ business }: UpdateBranchViewProps) {
  const {
    register,
    formState: { errors },
  } = useFormContext<updateBrach>();
  return (
    <>
      {/* General Information */}
          ...
          <div className="space-y-1.5 sm:col-span-2">
            ...
            <input
              ...
              {...register("address.street", {
                required: "La calle de la sucursal es obligatoria",
              })}
            />

            {errors.address?.street && (
              <ErrorAlert>{errors.address.street.message}</ErrorAlert>
            )}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              {...register("address.city", {
                required: "La ciudad de la sucursal es obligatoria",
              })}
            />

            {errors.address?.city && (
              <ErrorAlert>{errors.address.city.message}</ErrorAlert>
            )}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              {...register("address.state", {
                required: "El estado de la sucursal es obligatorio",
              })}
            />

            {errors.address?.state && (
              <ErrorAlert>{errors.address.state.message}</ErrorAlert>
            )}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              {...register("address.zipCode", {
                required: "El código postal de la sucursal es obligatorio",
              })}
            />

            {errors.address?.zipCode && (
              <ErrorAlert>{errors.address.zipCode.message}</ErrorAlert>
            )}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              {...register("address.country", {
                required: "El país de la sucursal es obligatorio",
              })}
            />

            {errors.address?.country && (
              <ErrorAlert>{errors.address.country.message}</ErrorAlert>
            )}
          </div>
        </div>
      </div>

...
import { useDeleteBranch } from "@/mutations/useMutationBranch";
import { useState } from "react";
import ModalLayout from "@/Components/Modals/ModalLayout";
import DeleteBranchView from "@/pages/dashboard/Owner/Branch/DeleteBranchView";
...
export default function BranchCard({ branch, businessId }: BranchCardProps) {
  const { mutate, isPending } = useDeleteBranch(branch._id);
  ...
  return (
   <>
      ...
      <div className="px-5 py-3 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between gap-2">
        <section>
          <button
            ...
            className="inline-flex cursor-pointer items-center gap-1.5 text-xs font-semibold text-red-700 hover:text-orange-700 transition-colors py-1.5 px-2 rounded-lg hover:bg-red-100"
          >
          ...
          <button
            onClick={() =>
              navigate(
                `/dashboard/business/${businessId}/branches/${branch._id}`,
              )
            }
    ...
    <ModalLayout open={open} setOpen={setOpen}>
      <DeleteBranchView setOpen={setOpen} isPending={isPending} branch={branch} handleDelete={handleDelete} />
    </ModalLayout>
   </>
  );
}
